...
j("btnLogin").onclick = async ()=>{
  try{
    const r = await post("/api/auth/login", { username: j("luser").value.trim(), password: j("lpass").value });
    location.href = "index.html";
  }catch(e){ j("lmsg").textContent = e.message }
}
j("btnRegister").onclick = async ()=>{
  try{
    ...
    location.href = "index.html";
  }catch(e){ j("rmsg").textContent = e.message }
}
