...
healthRouter.get("/", async (_req, res) => {
  let risk = null;
  try { risk = await riskSnapshot(); } catch {}
  res.json({
    ok: true,
...
healthRouter.get("/ready", (_req, res) => {
  ...
  res.status(dbUp ? 200 : 503).json({ ready: dbUp, ts: Date.now() });
});

// debug broker wiring
healthRouter.get("/debug/broker", async (req, res) => {
  const raw = (req.query.name || process.env.BROKER || "kite");
  const norm = normalizeBroker(raw);
  let mod = null;
  try { mod = await getBrokerAdapter(raw); } catch {}
  res.json({
    raw,
    normalized: norm,
