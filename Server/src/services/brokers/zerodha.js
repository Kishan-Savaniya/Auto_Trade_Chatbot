// Minimal Zerodha adapter compatible with your routes.
import fs from "fs";
...
export function connectMarketWS({ userId="default", instruments=[], onTick, onStatus }){
  ...
  ticker.on("ticks", (ticks=[])=>{ for(const t of ticks){ onTick?.({ symbol: symFromTok(t.instrument_token), ltp: t.last_price, ts: Date.now() }); }});
...
export async function modifyOrder(userId="default", brokerOrderId, changes={}){
  try{
    const k = kc(userId); const variety = changes.variety || "regular";
    const p = {};
    if (changes.quantity != null) p.quantity = Number(changes.quantity);
    if (changes.price    != null) p.price    = Number(changes.price);
    if (changes.order_type)       p.order_type = String(changes.order_type);
    if (changes.trigger_price!=null) p.trigger_price = Number(changes.trigger_price);
    if (changes.validity)         p.validity = String(changes.validity);
    await k.modifyOrder(variety, brokerOrderId, p);
    return { ok:true };
  }catch(e){ return { ok:false, error:e?.message||String(e) }; }
}

export async function cancelOrder(userId="default", brokerOrderId, { variety="regular" } = {}){
  try{ const k = kc(userId); await k.cancelOrder(variety, brokerOrderId); return { ok:true }; }
  catch(e){ return { ok:false, error:e?.message||String(e) }; }
}

export async function getPositions(userId="default"){
  try{
    const k = kc(userId); const r = await k.getPositions();
    return (r?.net || []).map(p=>({
      symbol:p.tradingsymbol, type:Number(p.quantity)>0?"LONG":Number(p.quantity)<0?"SHORT":"FLAT",
      qty:Math.abs(Number(p.quantity||0)), avgPrice:Number(p.average_price||0),
      ltp:Number(p.last_price||0), pnl:Number(p.pnl||0),
    }));
  }catch{ return []; }
}
...
export async function getOrders(userId="default"){
  try{
    const k = kc(userId); const list = await k.getOrders();
    return (list||[]).map(o=>({
      brokerOrderId:o.order_id, symbol:o.tradingsymbol, side:o.transaction_type,
      qty:Number(o.quantity||0), price:Number(o.average_price||o.price||0),
      status:o.status, createdAt:new Date(o.order_timestamp || Date.now()),
    }));
  }catch{ return []; }
}
